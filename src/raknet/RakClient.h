...
public:
	...
	*/
	...
	bool RPC( int* uniqueID, RakNet::BitStream *parameters, PacketPriority priority, PacketReliability reliability, unsigned orderingChannel, bool shiftTimestamp );
	/*
	...
	bool RPC( int* uniqueID, char *data, unsigned long bitLength, PacketPriority priority, PacketReliability reliability, unsigned orderingChannel, bool shiftTimestamp );
	/**
	 * Calls a C function on the server that the server already
	 * registered using RegisterAsRemoteProcedureCall Pass the data you
	 * want to pass to that function in parameters, or 0 for no data to
